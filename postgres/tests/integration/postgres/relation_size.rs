...
// (a) A nonexistent relation must return 0, not a fabricated positive size.
#[turso_macros::test]
fn test_pg_relation_size_nonexistent_returns_zero(db: TempDatabase) {
    let conn = db.connect_postgres();
    conn.execute("CREATE TABLE t (id integer PRIMARY KEY)")
        .unwrap();
    conn.set_relation_size_fn(Arc::new(|| 4096));

    // Even with a size hook installed, an unknown relation reports 0.
    assert_eq!(
        query_integer(&conn, "SELECT pg_table_size('does_not_exist')"),
        [0]
    );
    assert_eq!(
        query_integer(&conn, "SELECT pg_relation_size('does_not_exist')"),
        [0]
    );
    assert_eq!(
        query_integer(&conn, "SELECT pg_indexes_size('does_not_exist')"),
        [0]
    );
}
