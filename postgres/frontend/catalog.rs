...
impl PgTypeCursor {
    fn make_type_row(t: &PgTypeInfo) -> Vec<Value> {
        vec![
            ...
            Value::build_text(t.name), // typname (internal name, e.g. "int4", matching PostgreSQL)
...
///
/// Returns the OIDs of the ancestor partitions of a partitioned table. Turso
...
impl InternalVirtualTable for PgPartitionAncestorsTable {
    ...
    fn sql(&self) -> String {
        "CREATE TABLE pg_partition_ancestors (pg_partition_ancestors INTEGER, regclass TEXT HIDDEN)"
            .to_string()
    }
    ...
    fn new(conn: Arc<Connection>) -> Self {
        ...
        for (table_name, _) in user_tables_sorted(&schema) {
            cursor.rows.push(vec![
                Value::Text("turso".into()),            // table_catalog
...
impl InfoSchemaRoutinesCursor {
    fn new(_conn: Arc<Connection>) -> Self {
        // Derived from the shared function table so the view can never
        // disagree with what `resolve_scalar` actually accepts.
        let rows = crate::functions::SCALAR_FUNCTIONS
            .iter()
            .map(|name| {
                vec![
                    Value::Text("turso".into()),
...
// Reports the PRIMARY KEY constraint for each user table that actually has
// one (compat shim; only PKs are modeled). Tables without a PK are omitted
// rather than fabricated — ORMs inspect this view for migration strategy, and
// lying about PK *presence* is worse than omitting the detail.
...
impl InternalVirtualTable for InfoSchemaTableConstraintsTable {
    ...
    fn open(
        ...
        Ok(Arc::new(RwLock::new(
            InfoSchemaTableConstraintsCursor::new(conn),
        )))
    }
...
impl InfoSchemaTableConstraintsCursor {
    fn new(conn: Arc<Connection>) -> Self {
        ...
        let schema = conn.current_schema();
        for (table_name, table) in user_tables_sorted(&schema) {
            // Only report a PRIMARY KEY when the table actually has one. Tables
            // without a PK must not be advertised as having one — ORMs inspect
            // this view to decide migration strategy, and a fabricated PK is
            // worse than an omitted detail.
            ...
            cursor.rows.push(vec![
                Value::Text("turso".into()),
                Value::Text("public".into()),
                Value::Text(format!("{table_name}__pkey").into()),
                Value::Text("turso".into()),
                Value::Text("public".into()),
                Value::Text(table_name.clone().into()),
