...
fn param_type_for(sql: &str, i: usize, cols: &Option<HashMap<String, Type>>) -> Type {
    let param = format!("${i}");
...
fn insert_value_type(sql: &str, i: usize, cols: &Option<HashMap<String, Type>>) -> Option<Type> {
    let upper = sql.to_uppercase();
